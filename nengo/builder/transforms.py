...
def build_dense(model, transform, sig_in, decoders=None, encoders=None,
                rng=np.random):
    ...
    weighted = Signal(np.zeros(transform.size_out if encoders is None else
                               weights.shape[0]),
...
def build_convolution(model, transform, sig_in, decoders=None, encoders=None,
                      rng=np.random):
    if decoders is not None:
        raise BuildError(
            "Applying convolution transform to decoded connections is "
            "not supported")
    if encoders is not None:
        raise BuildError(
            "Applying encoders to convolution transform is not supported")
